...
namespace System
{
    public class IPLite
    {
        ...
        public IPLite(SqlCommand cmd, string ipAddress)
        {
            REMOTE_ADDR = new IpInfo(cmd, ipAddress.Trim());
        }

        public IPLite(SqlCommand cmd)
        {
            GetData(cmd);
        }

        public void GetData(SqlCommand cmd)
        {
            // REMOTE_ADDR
    ...
    public class IpInfo
    {
        ...
        public IpInfo(SqlCommand cmd, string ipAddress)
        {
        ...
        {
            cmd.CommandText = string.Format("select * from ip2location where ip_from <= {0} and ip_to >= {0}", IpNumber);

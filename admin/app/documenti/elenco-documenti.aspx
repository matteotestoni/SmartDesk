        			...
        			<div class="dropdown-pane" id="dropdowntipodocumenti" data-dropdown data-hover="true" data-hover-pane="true">
        			  <ul class="no-bullet">
        		      	<% for (int i = 0; i < dtDocumentiTipo.Rows.Count; i++){ %>
          			  	<li><a href="/admin/app/documenti/scheda-documenti.aspx?azione=new&DocumentiTipo_Ky=<%=dtDocumentiTipo.Rows[i]["DocumentiTipo_Ky"].ToString()%>" class="tiny"><i class="fa-duotone fa-square-plus fa-lg fa-fw"></i><%=dtDocumentiTipo.Rows[i]["DocumentiTipo_Descrizione"].ToString()%></a></li>
                  		<% } %>
        			  </ul>
        			</div>
        		</div>
    	...
    	<tbody>
		    <%for (int i = 0; i < dtDocumenti.Rows.Count; i++){ %>
		        <tr class="riga<%=i%2%>">
		          ...
		          <td>
		            <a href="/admin/app/documenti/scheda-documenti.aspx?Documenti_Ky=<%=dtDocumenti.Rows[i]["Documenti_Ky"].ToString()%>&azione=modifica"><%=dtDocumenti.Rows[i]["Documenti_Ky"].ToString()%></a> 
		          </td>
		          <td class="text-left"><strong><%=dtDocumenti.Rows[i]["DocumentiTipo_Codice"].ToString()%></strong></td>
		          <td class="large-text-center small-text-left"><a href="/admin/app/documenti/scheda-documenti.aspx?Documenti_Ky=<%=dtDocumenti.Rows[i]["Documenti_Ky"].ToString()%>&azione=modifica"><strong><%=dtDocumenti.Rows[i]["Documenti_Numero"].ToString()%></strong></a></td>
		          <td class="large-text-center small-text-left"><a href="/admin/app/documenti/scheda-documenti.aspx?Documenti_Ky=<%=dtDocumenti.Rows[i]["Documenti_Ky"].ToString()%>&azione=modifica"><i class="fa-duotone fa-calendar-days fa-fw"></i><strong><%=dtDocumenti.Rows[i]["Documenti_Data_IT"].ToString()%></strong></a></td>
		          <td>
		          	<div class="width300">

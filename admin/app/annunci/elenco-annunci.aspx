    	...
    	<tbody>
		    <%for (int i = 0; i < dtGridData.Rows.Count; i++){ %>
		        <tr class="riga<%=i%2%>">
		          ...
		          <td><div class="width300"><img src="/img/flags/<%=dtGridData.Rows[i]["Nazioni_Codice"].ToString()%>.png" hspace="4" vspace="2" align="left"><%=dtGridData.Rows[i]["Nazioni_Codice"].ToString()%>/<%=dtGridData.Rows[i]["Regioni_Regione"].ToString()%>/<%=dtGridData.Rows[i]["Comuni_Comune"].ToString()%></div></td>
        			<td class="text-center">

    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      if (Smartdesk.Login.Verify){
          dtLogin = Smartdesk.Data.Read("Utenti_Vw","Utenti_Ky", Smartdesk.Session.CurrentUser.ToString());          
            ...
            strTipo=Smartdesk.Current.Request("tipo");
            strSorgente=Smartdesk.Current.Request("sorgente");
            switch (strTipo){
              case "scaduti":
                strSQL = "UPDATE Attivita SET Attivita_Scadenza=GETDATE()-10 WHERE Attivita_Ky=" + strAttivita_Ky;
                break;
              case "appenascaduti":
                strSQL = "UPDATE Attivita SET Attivita_Scadenza=GETDATE() WHERE Attivita_Ky=" + strAttivita_Ky;
                break;
              case "prossimi":
                strSQL = "UPDATE Attivita SET Attivita_Scadenza=GETDATE()+7 WHERE Attivita_Ky=" + strAttivita_Ky;
                break;
              case "futuri":
                strSQL = "UPDATE Attivita SET Attivita_Scadenza=GETDATE()+60 WHERE Attivita_Ky=" + strAttivita_Ky;
                break;
            }

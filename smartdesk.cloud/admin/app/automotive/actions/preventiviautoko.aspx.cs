      ...
      if (Smartdesk.Login.Verify){
            ...
            if (strOpportunita_Ky!=null && strOpportunita_Ky.Length>0 && strOpportunita_Ky!="0"){
              ...
              if (dtOpportunita.Rows.Count>0){
                strSQL = "UPDATE Opportunita SET OpportunitaStati_Ky=4, Opportunita_DateUpdate=GETDATE(), Opportunita_UserUpdate=" + Smartdesk.Session.CurrentUser.ToString() + " WHERE Opportunita_Ky=" + strOpportunita_Ky;
            ...
            //ok
            strSQL = "UPDATE PreventiviAuto SET PreventiviAutoStati_Ky=2, PreventiviAuto_DateUpdate=GETDATE(), PreventiviAuto_UserUpdate=" + Smartdesk.Session.CurrentUser.ToString() + " WHERE PreventiviAuto_Ky=" + strPreventiviAuto_Ky;
  					new Smartdesk.Sql().SQLScriptExecuteNonQuery(strSQL);

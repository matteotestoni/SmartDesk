      ...
      if (Smartdesk.Login.Verify){
            ...
            if (strOpportunita_Ky!=null && strOpportunita_Ky.Length>0 && strOpportunita_Ky!="0"){
              ...
              if (dtOpportunita.Rows.Count>0){
                strSQL = "UPDATE Opportunita SET OpportunitaStati_Ky=6, Opportunita_DateUpdate=GETDATE(), Opportunita_UserUpdate=" + Smartdesk.Session.CurrentUser.ToString() + " WHERE Opportunita_Ky=" + strOpportunita_Ky;
      					new Smartdesk.Sql().SQLScriptExecuteNonQuery(strSQL);
                strLead_Ky=dtOpportunita.Rows[0]["Lead_Ky"].ToString();
                if (strLead_Ky!=null && strLead_Ky.Length>0){
                  strSQL = "UPDATE Lead SET LeadStato_Ky=3, Lead_DateUpdate=GETDATE(), Lead_UserUpdate=" + Smartdesk.Session.CurrentUser.ToString() + " WHERE Lead_Ky=" + strLead_Ky;
        					new Smartdesk.Sql().SQLScriptExecuteNonQuery(strSQL);
                }
              }
            }
            ...
            //ok
            strSQL = "UPDATE PreventiviAuto SET PreventiviAutoStati_Ky=3, PreventiviAuto_DateUpdate=GETDATE(), PreventiviAuto_UserUpdate=" + Smartdesk.Session.CurrentUser.ToString() + " WHERE PreventiviAuto_Ky=" + strPreventiviAuto_Ky;
  					new Smartdesk.Sql().SQLScriptExecuteNonQuery(strSQL);
